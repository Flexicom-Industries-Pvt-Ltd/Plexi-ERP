...
type ValvomaticRun = {
  ...
  outputBagQty: number;
  valvomaticMachine: { id: string; name: string };
  operator: { id: string; name: string };
...
const emptyInputs = (): ValvomaticInputs => ({
  inputRollId: "",
  rollQty: 0,
  yarnItemId: "",
  yarnQty: 0,
...
export function ValvomaticProductionClient() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [selectedDate, setSelectedDate] = useState(new Date().toISOString().slice(0, 10));
  const [shifts, setShifts] = useState<any[]>([]);
  const [operators, setOperators] = useState<any[]>([]);
  const [machines, setMachines] = useState<ValvomaticMachine[]>([]);
  const [planLines, setPlanLines] = useState<PlanLine[]>([]);
  const [activeRuns, setActiveRuns] = useState<ValvomaticRun[]>([]);
  ...
  const [hasValvomaticPlans, setHasValvomaticPlans] = useState(true);
  ...
  const [completingRun, setCompletingRun] = useState<ValvomaticRun | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/production/operators")
      .then((r) => (r.ok ? r.json() : []))
      .then(setOperators);
    fetch("/api/production/valvomatic/input-rolls")
      .then((r) => (r.ok ? r.json() : []))
      .then(setInputRolls);
    fetch("/api/production/valvomatic/input-materials?type=yarn")
      .then((r) => (r.ok ? r.json() : []))
      .then(setYarnItems);
    fetch("/api/production/valvomatic/input-materials?type=pp")
      .then((r) => (r.ok ? r.json() : []))
      .then(setPpItems);
    fetch("/api/production/valvomatic/input-materials?type=lpp")
      .then((r) => (r.ok ? r.json() : []))
      .then(setLppItems);
    fetch("/api/production/valvomatic/output-items")
      .then((r) => (r.ok ? r.json() : []))
      .then(setOutputItems);
  }, []);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [machinesRes, plansRes, runsRes] = await Promise.all([
        fetch("/api/production/valvomatic/machines"),
        fetch(`/api/production/plans?phase=VALVOMATIC&dateFrom=${selectedDate}&dateTo=${selectedDate}`),
        fetch("/api/production/valvomatic/runs?activeOnly=true"),
      ]);
      ...
      if (plansRes.ok) {
        ...
        for (const plan of plans) {
          ...
          for (const line of plan.lines || []) {
            if (line.phase === "VALVOMATIC" && (!line.finishingRoute || line.finishingRoute === "VALVOMATIC")) {
              lines.push({
                ...line,
                plan: {
        ...
        setPlanLines(lines);
        setHasValvomaticPlans(lines.length > 0);
      }
      if (runsRes.ok) setActiveRuns(await runsRes.json());
    } catch {
      toast.error("Failed to load valvomatic production data");
    } finally {
      setLoading(false);
    }
  }, [selectedDate]);
  ...
  const handleStart = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!startForm.planLineId || !startForm.valvomaticMachineId || !startForm.operatorId) {
      toast.error("Select plan line, machine, and operator");
      return;
    }
    setSaving(true);
    try {
      const res = await fetch("/api/production/valvomatic/runs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          planLineId: startForm.planLineId,
          valvomaticMachineId: startForm.valvomaticMachineId,
      ...
      toast.success("Valvomatic run started");
      setStartForm({
        planLineId: "",
        valvomaticMachineId: "",
        operatorId: "",
  ...
  const openComplete = (run: ValvomaticRun) => {
  ...
  const handleComplete = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/production/valvomatic/runs/${completingRun.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "complete",
      ...
      toast.success(result.inventory?.message || "Valvomatic run completed");
  ...
  const onPlanLineSelect = (lineId: string) => {
    ...
    setStartForm({
      ...
      valvomaticMachineId: line?.machine?.id || startForm.valvomaticMachineId,
  ...
  const updateStartInputs = (patch: Partial<ValvomaticInputs>) => {
    setStartForm({ ...startForm, inputs: { ...startForm.inputs, ...patch } });
  };

  const updateCompleteInputs = (patch: Partial<ValvomaticInputs>) => {
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
              ...
              Valvomatic Production
            </h1>
            <p className="text-sm text-slate-500 mt-1">
              Multi-input finishing — roll, yarn, and PP/LPP to finished bags with full run tracking.
            </p>
          </div>
      ...
      {!hasValvomaticPlans && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-amber-900">No Valvomatic route on plan today</p>
            <p className="text-sm text-amber-800 mt-1">
              Add a VALVOMATIC plan line with finishing route Valvomatic for {selectedDate}.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        <h2 className="font-semibold text-slate-800">Valvomatic Machines ({machines.length})</h2>
        {machines.length === 0 ? (
          <p className="text-sm text-slate-500">No Valvomatic machines found. Add machines under a Valvomatic section.</p>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          <h2 className="font-semibold text-slate-800 flex items-center gap-2">
            <Play className="h-4 w-4 text-primary" /> Start Valvomatic Run
          </h2>
          <form onSubmit={handleStart} className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Plan Line</label>
              <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm disabled:bg-slate-50" value={startForm.planLineId} onChange={(e) => onPlanLineSelect(e.target.value)} required disabled={!hasValvomaticPlans}>
                <option value="">{hasValvomaticPlans ? "Select VALVOMATIC plan line…" : "No Valvomatic plan today"}</option>
                {planLines.map((line) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm disabled:bg-slate-50" value={startForm.valvomaticMachineId} onChange={(e) => setStartForm({ ...startForm, valvomaticMachineId: e.target.value })} required disabled={!hasValvomaticPlans}>
              ...
              <div>
                ...
                <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm disabled:bg-slate-50" value={startForm.operatorId} onChange={(e) => setStartForm({ ...startForm, operatorId: e.target.value })} required disabled={!hasValvomaticPlans}>
            ...
            <div>
              ...
              <input type="number" min={0} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm" value={startForm.targetQty} onChange={(e) => setStartForm({ ...startForm, targetQty: parseFloat(e.target.value) || 0 })} required disabled={!hasValvomaticPlans} />
            </div>
            ...
            <div className="border border-slate-100 rounded-lg p-3 space-y-3 bg-slate-50/50">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.inputRollId || ""} onChange={(e) => onRollSelect(e.target.value)} disabled={!hasValvomaticPlans}>
                ...
                <div>
                  ...
                  <input type="number" min={0} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.rollQty} onChange={(e) => updateStartInputs({ rollQty: parseFloat(e.target.value) || 0 })} disabled={!hasValvomaticPlans} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">Yarn (Bobbin)</label>
                  <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.yarnItemId || ""} onChange={(e) => updateStartInputs({ yarnItemId: e.target.value })} disabled={!hasValvomaticPlans}>
                ...
                <div>
                  ...
                  <input type="number" min={0} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.yarnQty} onChange={(e) => updateStartInputs({ yarnQty: parseFloat(e.target.value) || 0 })} disabled={!hasValvomaticPlans} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">PP Material</label>
                  <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.ppItemId || ""} onChange={(e) => updateStartInputs({ ppItemId: e.target.value })} disabled={!hasValvomaticPlans}>
                ...
                <div>
                  ...
                  <input type="number" min={0} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.ppQty} onChange={(e) => updateStartInputs({ ppQty: parseFloat(e.target.value) || 0 })} disabled={!hasValvomaticPlans} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-1">LPP Material</label>
                  <select className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.lppItemId || ""} onChange={(e) => updateStartInputs({ lppItemId: e.target.value })} disabled={!hasValvomaticPlans}>
                ...
                <div>
                  ...
                  <input type="number" min={0} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-white" value={startForm.inputs.lppQty} onChange={(e) => updateStartInputs({ lppQty: parseFloat(e.target.value) || 0 })} disabled={!hasValvomaticPlans} />
                </div>
              </div>
            </div>

            <button type="submit" disabled={saving || !hasValvomaticPlans} className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium disabled:opacity-50">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          {inputRolls.length === 0 ? (
            <p className="text-sm text-slate-500">No rolls available for valvomatic input.</p>
          ) : (
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
        ...
        {activeRuns.length === 0 ? (
          <p className="text-sm text-slate-500">No active valvomatic runs.</p>
        ) : (
          <div className="space-y-3">
            {activeRuns.map((run) => (
              <div key={run.id} className="flex flex-wrap items-center justify-between gap-3 border border-slate-100 rounded-lg p-4">
                <div>
                  <p className="font-medium text-slate-800">{run.valvomaticMachine.name} · {run.operator.name}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-5 border-b">
              <h3 className="font-semibold text-slate-800">Complete Valvomatic Run</h3>
              <p className="text-sm text-slate-500 mt-1">{completingRun.valvomaticMachine.name}</p>
            </div>

...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/production/dashboard")
      ...
      .then((data) => data?.summary && setProduction(data.summary))
      .catch(() => {});
  }, []);
  ...
  const stats = [
    ...
    },
    {
      ...
      value: production ? String(production.activeProductionOrders) : "—",
      change: production
        ? `${production.achievementPercent}% achievement today`
...
function StatCard({ stat }: { stat: { title: string; value: string; change: string; icon: typeof Factory; color: string; bg: string } }) {
  return (
    <Card className="glass hover-lift border-0 ring-1 ring-black/5 relative overflow-hidden group h-full">

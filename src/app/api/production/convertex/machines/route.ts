  ...
  try {
    const machines = await db.machine.findMany({
      where: {
        ...
        section: {
          OR: [
            { code: { equals: "CVX", mode: "insensitive" } },
            { name: { contains: "Convertex", mode: "insensitive" } },
          ],
        },
      },
      orderBy: { name: "asc" },
    ...
    const activeRuns = await db.convertexProductionRun.findMany({
      where: { productionRun: { endedAt: null } },
      include: {
        productionRun: { select: { id: true, targetQty: true, startedAt: true } },
        operator: { select: { id: true, name: true } },
        convertexMachine: { select: { id: true } },
        inputMaterial: { select: { id: true, code: true, name: true } },
      },
    });

    const runByMachine = new Map(activeRuns.map((r) => [r.convertexMachineId, r]));
      ...
      return {
        ...
        activeRun: activeRun
          ? {
              ...
              inputMaterial: activeRun.inputMaterial,
            }
          : null,
      };
    });
    ...
    console.error("Error fetching convertex machines:", error);
    return NextResponse.json({ error: "Failed to fetch convertex machines" }, { status: 500 });
  }
}

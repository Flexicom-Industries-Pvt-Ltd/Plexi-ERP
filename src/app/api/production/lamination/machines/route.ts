  ...
  try {
    const machines = await db.machine.findMany({
      where: {
        ...
        section: {
          OR: [
            { code: { equals: "LAM", mode: "insensitive" } },
            { name: { contains: "Lamination", mode: "insensitive" } },
          ],
        },
      },
      orderBy: { name: "asc" },
    ...
    const activeRuns = await db.laminationProductionRun.findMany({
      where: { productionRun: { endedAt: null } },
      include: {
        productionRun: { select: { id: true, targetQty: true, startedAt: true } },
        operator: { select: { id: true, name: true } },
        laminationMachine: { select: { id: true } },
        inputRoll: { select: { id: true, rollNumber: true, rollType: true } },
      },
    });

    const runByMachine = new Map(activeRuns.map((r) => [r.laminationMachineId, r]));
    ...
    console.error("Error fetching lamination machines:", error);
    return NextResponse.json({ error: "Failed to fetch lamination machines" }, { status: 500 });
  }
}

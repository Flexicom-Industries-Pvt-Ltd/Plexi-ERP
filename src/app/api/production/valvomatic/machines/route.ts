  ...
  try {
    const machines = await db.machine.findMany({
      where: {
        ...
        section: {
          OR: [
            { code: { equals: "VLV", mode: "insensitive" } },
            { name: { contains: "Valvomatic", mode: "insensitive" } },
          ],
        },
      },
      orderBy: { name: "asc" },
    ...
    const activeRuns = await db.valvomaticProductionRun.findMany({
      where: { productionRun: { endedAt: null } },
      include: {
        productionRun: { select: { id: true, targetQty: true, startedAt: true } },
        operator: { select: { id: true, name: true } },
        valvomaticMachine: { select: { id: true } },
      },
    });

    const runByMachine = new Map(activeRuns.map((r) => [r.valvomaticMachineId, r]));
    ...
    console.error("Error fetching valvomatic machines:", error);
    return NextResponse.json({ error: "Failed to fetch valvomatic machines" }, { status: 500 });
  }
}
